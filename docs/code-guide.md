...
- **`vcctl.py`** (`saga-ctl`) — control del ciclo de vida: `start/stop/status/restart`. Mata por
  **basename/ruta exacta** leyendo `/proc` (nunca `pkill -f`, que se auto-mataría y pegaría en el
  `claude` CLI). Rutea por `SAGA_TRANSPORT`: **`_start_room()`** (default, Ciclo 4) orquesta server
  nativo → cerebro → orbe → worker → dispatch → browser con readiness por pieza; `_start_livekit()`
  es el fallback console; el flujo clásico si `VOICE_LIVEKIT=0`. Helpers room: `_node_ip()` (IP LAN
...
- **`lk/agent.py`** — el **worker** del modo default. Mismo código, dos transportes según el
  subcomando: **`start` → ROOM** (default, Ciclo 4: worker conectado a `livekit-server`, el audio
  llega por el track del browser) y **`console` → audio local** (fallback dev, sin server). El modo
  lo decide `_CONSOLE_MODE` (`"console" in sys.argv`). Se registra como agente NOMBRADO con
  ...
  VAD-aware) y `_arm_busy` (18s watchdog de turno colgado en thinking). BVC (noise cancellation) se
  **gatea a console** (requiere LiveKit Cloud → falla en el server self-hosted del modo room). En
  room además levanta orbe + precalienta Claude (dedup-safe). Plugins LiveKit importados a **nivel
...
- **`vc/config.py`** — **fuente única** de paths, flags, voces, system prompt (agéntico: usa las
  ...
  `LIVEKIT_SIGNAL_PORT` (7880), y `SAGA_TRANSPORT` (`room` default / `console`). Las keys solo
...
- **`vc/orb.py`** — cliente del orbe: levanta el server si hace falta y manda el **estado** por HTTP
  no bloqueante (cola + keep-alive). No manda nivel de audio: en console el orbe anima stylized; en
  room late con el track TTS vía Web Audio en el browser (no por este canal).
...
- **`orb/orb_server.py`** — server persistente (HTTP + SSE), stdlib. Sirve la página, emite estado
  ...
  (`livekit.api.AccessToken` + `RoomConfiguration`/`RoomAgentDispatch` para el dispatch explícito);
  `livekit.api` se importa **lazy** dentro del handler → en console el módulo sigue stdlib-only. Sirve
...
- **`orb/orb.html`** — render Three.js del orbe (estados acoplados por bloom) **+ cliente LiveKit**
  ...
  (U5)** el orbe late con el nivel REAL de la voz (`window.__ttsLevel()`) — en console era imposible
  (audio en otro proceso). Cuidado: colores de fondo en **sRGB** (`THREE.SRGBColorSpace`), sin eso el
  bloom revienta a blanco.
